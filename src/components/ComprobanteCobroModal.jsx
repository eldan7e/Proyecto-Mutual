...
export default function ComprobanteCobroModal({ isOpen, onClose, cobroData }) {
  ...
  function handleEnviarWhatsApp() {
    ...
    text += `💳 *Medio:* ${medio_pago} (${observaciones || 'Sin observaciones'})\n\n`;
    text += `*DETALLE DE IMPUTACIÓN DE CONCEPTOS (FIFO):*\n`;
    desgloses.forEach(d => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Comprobante Oficial de Cobro" maxWidth="660px">
      
      {/* CSS para Impresión Limpia de Página Completa */}
      <style>{`
        @media print {
          body * {
          ...
          .no-print, button, .modal-backdrop, .modal-header, .modal-footer {
      ...
      <div id="print-recibo" style={{ background: 'var(--surface)', padding: '28px', borderRadius: '18px', border: '1px solid var(--border-light)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid var(--border-light)', paddingBottom: '18px', marginBottom: '22px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            <img 
              ...
              onError={(e) => {
                // Fallback elegante en caso de error de imagen
                e.target.style.display = 'none';
              }}
            />
            <div>
              <h2 style={{ fontSize: '20px', fontWeight: 900, color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.02em' }}>
                MUTUAL AUNAR
              </h2>
          ...
          <div style={{ textAlign: 'right', background: 'rgba(16,185,129,0.08)', padding: '10px 16px', borderRadius: '12px', border: '1px solid rgba(16,185,129,0.2)' }}>
            <span style={{ fontSize: '15px', fontWeight: 900, color: '#10b981', display: 'block' }}>
              {reciboNumero}
            </span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '22px', background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '14px', border: '1px solid var(--border-light)' }}>
          <div>
            <span style={{ fontSize: '11px', color: 'var(--text-secondary)', textTransform: 'uppercase', fontWeight: 800, letterSpacing: '0.05em' }}>Grupo Pagador</span>
            <p style={{ fontSize: '16px', fontWeight: 900, margin: '4px 0 0 0', color: 'var(--text-primary)' }}>
              Grupo N° {numero_grupo}
            </p>
            <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', margin: '2px 0 0 0' }}>
              {nombre_titular}
            </p>
          </div>

          <div>
            <span style={{ fontSize: '11px', color: 'var(--text-secondary)', textTransform: 'uppercase', fontWeight: 800, letterSpacing: '0.05em' }}>Forma de Cobro</span>
            <p style={{ fontSize: '16px', fontWeight: 900, margin: '4px 0 0 0', color: '#10b981' }}>
              {medio_pago}
            </p>
            <p style={{ fontSize: '12px', fontWeight: 500, color: 'var(--text-secondary)', margin: '2px 0 0 0' }}>
              {observaciones || 'Cobro registrado e imputado en cuenta corriente'}
            </p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '22px' }}>
          <h4 style={{ fontSize: '13px', fontWeight: 800, marginBottom: '10px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {desgloses.length === 0 ? (
                <tr>
                  <td colSpan="4" style={{ padding: '14px', textAlign: 'center', color: 'var(--text-secondary)' }}>
                    Pago asignado como crédito / saldo a favor general de la cuenta corriente.
                  </td>
                </tr>
              ) : (
                desgloses.map((d, idx) => {
                  ...
                  return (
                    <tr key={idx} style={{ borderBottom: '1px solid var(--border-light)' }}>
                      ...
                      <td style={{ padding: '10px 12px', textAlign: 'right', color: '#ef4444', fontWeight: 600 }}>{formatMoney(d.pagoAplicadoInteres)}</td>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '16px', borderTop: '2px solid var(--border-light)' }}>
          <div>
            <span style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 700, textTransform: 'uppercase' }}>Saldo Restante del Grupo:</span>
            <span style={{ fontSize: '15px', fontWeight: 900, marginLeft: '8px', color: saldo_restante > 5 ? '#ef4444' : '#10b981' }}>
              {formatMoney(saldo_restante)}
            </span>
          </div>

          <div style={{ textAlign: 'right' }}>
            <span style={{ fontSize: '11px', color: 'var(--text-secondary)', fontWeight: 800, textTransform: 'uppercase', display: 'block' }}>TOTAL COBRADO</span>
