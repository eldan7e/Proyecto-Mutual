...
async function enrichConsumosWithAudit(consumos, periodo) {
  ...
  for (let i = 0; i < lineNumbers.length; i += 200) {
    ...
    const chunkLineas = await fetchAllPaginated(() =>
      supabase
        .from('lineas')
        .select('*, planes_abonos (*), socios:socios!lineas_socio_id_fkey(*, grupo_socio(numero_grupo)), responsable:socios!lineas_socio_responsable_id_fkey(*, grupo_socio(numero_grupo)), proveedores:proveedor_id (*)')
